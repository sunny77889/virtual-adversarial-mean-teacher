...
def run(data_seed=0):
    ...
    model['training_length'] = 10000
    model['max_consistency_cost'] = 50.0

    tensorboard_dir = model.save_tensorboard_graph()
    LOG.info("Saved tensorboard graph to %r", tensorboard_dir)

    mnist = MNIST(data_seed, n_labeled, n_extra_unlabeled)
    training_batches = minibatching.training_batches(mnist.training, n_labeled_per_batch=50)
    evaluation_batches_fn = minibatching.evaluation_epoch_generator(mnist.evaluation)
